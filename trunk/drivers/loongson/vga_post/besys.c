...
****************************************************************************/
#ifdef SIMPLIFY_BIOS_EMU
static inline u8 *BE_memaddr(u32 addr)
{
	if (addr >= 0xC0000 && addr <= _BE_env.biosmem_limit) {	/* MEM ACCESS */
		return (u8*)(_BE_env.biosmem_base + addr - 0xC0000);
	} else if (addr >= 0xA0000 && addr <= 0xBFFFF) {	/* PCI ACCESS */
		return (u8*)(_BE_env.busmem_base + addr - 0xA0000);
	} else if (addr <= M.mem_size - 1) {	/* MEM ACCESS */
...
#define IS_CMOS_PORT(port)  0//(0x70 <= port && port <= 0x71)
/*#define IS_VGA_PORT(port)   (_BE_env.emulateVGA && 0x3C0 <= port && port <= 0x3DA)*/
#define IS_VGA_PORT(port)   0//(0x3C0 <= port && port <= 0x3DA)
#define IS_PCI_PORT(port)   (0xCF8 <= port && port <= 0xCFF)
#define IS_SPKR_PORT(port)  0//(port == 0x61)
#else
#define IS_TIMER_PORT(port) (0x40 <= port && port <= 0x43)
