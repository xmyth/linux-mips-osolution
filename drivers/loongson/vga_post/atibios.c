...
#include "include/biosemui.h"
#include <linux/types.h>
#include <linux/module.h>
#include <linux/io.h>
#include <linux/pci.h>

#include "include/z9sl10810fw.h"
#ifdef CFG_LEGACY_VGA_SUPPORT
pci_dev_t vga_dev = 0;
#endif
...
static void PCI_fixupIObase(pci_dev_t pcidev, int reg, u32 * base)
{
	...
	if(*base & 0x1){
		//video_card_io_phy = pci_io_to_phys(pcidev,((*base)&~ 0x1));
	}
}
...
void *PCI_mapBIOSImage(pci_dev_t pcidev)
{
	...
	for(i = 0; i < 5; i++){
		pci_read_config_dword(pcidev, (PCI_BASE_ADDRESS_0 + (i<<2)), &addr);
		if(addr & 0x1){
			PCI_fixupIObase(pcidev, (PCI_BASE_ADDRESS_0 + (i<<2)), &addr);
			break;
		}
	}
	...
	*/
	...
	//pci_write_config_dword(pcidev, PCI_ROM_ADDRESS, pci_phys_to_mem(pcidev,BIOSImagePhys) | 0x1);
...
static int PCI_postController(pci_dev_t pcidev, BE_VGAInfo * VGAInfo)
{
	...
	if ((copyOfBIOS = kmalloc(BIOSImageLen, GFP_KERNEL)) == NULL) {
	...
	VGAInfo->function = PCI_FUNC(pcidev->devfn);
	VGAInfo->device = PCI_SLOT(pcidev->devfn);
	VGAInfo->bus = 0;
...
****************************************************************************/
int BootVideoCardBIOS(pci_dev_t pcidev, int cleanUp)
{
	BE_VGAInfo *VGAInfo;

	printf("videoboot: Booting PCI video card bus %d, function %d, device %d\n",
	     0, PCI_FUNC(pcidev->devfn), PCI_SLOT(pcidev->devfn));

	/*Initialise the x86 BIOS emulator*/
	if ((VGAInfo = kmalloc(sizeof(*VGAInfo), GFP_KERNEL)) == NULL) {
		printf("videoboot: Out of memory!\n");
		return false;
	}
