...
#define readb_le(base)	*((u8*)(base))
#define readw_le(base)	((((u32)base)&1)? ((u16)readb_le(base) | ((u16)readb_le((base) + 1) << 8)) : (*((u16*)(base))))
#define readl_le(base)	((((u32)base)&3)? ((u32)readb_le((base) + 0) | ((u32)readb_le((base) + 1) << 8) | \
			((u32)readb_le((base) + 2) << 16) | ((u32)readb_le((base) + 3) << 24)) : (*((u32*)(base))))
#define writeb_le(base, v)	*((u8*)(base)) = (v)
#define writew_le(base, v)	\
do {	\
	if(((u32)base)&1) {	\
		writeb_le(base + 0, v & 0xff);	\
		writeb_le(base + 1, (v >> 8) & 0xff);	\
	} else {	\
		*((u16*)(base)) = (v); }	\
} while (0)
#define writel_le(base, v)  	\
do {	\
	if(((u32)base)&3) {	\
		writeb_le(base + 0, v & 0xff);	\
		writeb_le(base + 1, (v >> 8) & 0xff);	\
