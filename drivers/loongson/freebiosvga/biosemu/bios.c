    ...
    int intno)
{
	...
	{                                         /* Localise */
	    ...
	    /* Programme the mode */
	    outb(cgamode & 0x37, ioport + 4);   /* Turn off screen */
	    for (i = 0; i < 0x10; i++) {
		tmp = BE_rdb( regvals + i);
		outb(i, ioport);
		outb(tmp, ioport + 1);
	    }
	    outb(cgacolour, ioport + 5);        /* Select colour mode */
	    outb(cgamode, ioport + 4);          /* Turn on screen */
	}
	break;
	...
	{                                         /* Localise */
	    ...
	    outb(0x0A, ioport);
	    outb(M.x86.R_CH, ioport + 1);
	    outb(0x0B, ioport);
	    outb(M.x86.R_CL, ioport + 1);
	}
	break;
	...
	{                                         /* Localise */
	    ...
	    outb(0x0E, ioport);
	    outb(offset >> 8, ioport + 1);
	    outb(0x0F, ioport);
	    outb(offset&0xFF,ioport + 1);
	}
	break;
	...
	{                                         /* Localise */
	    ...
	    outb(0x0C, ioport);
	    outb(start>>8,ioport + 1);
	    outb(0x0D, ioport);
	    outb(start&0xFF,ioport + 1);
...
static void  int1A(int unused)
{
    ...
    struct pci_dev *pdev = _BE_env.vgaInfo.pciInfo;
    ...
    switch (M.x86.R_AX) {
        ...
        case 0xB102:                    /* Find PCI device */
            ...
            if (M.x86.R_DX == pdev->vendor &&
                    M.x86.R_CX == pdev->device &&
                    M.x86.R_SI == 0) {
        ...
        case 0xB108:                    /* Read configuration byte */
            ...
            if (M.x86.R_BX == pciSlot) {
                M.x86.R_AH = SUCCESSFUL;
		pci_read_config_byte(pdev,M.x86.R_DI,&M.x86.R_CL);
        ...
        case 0xB109:                    /* Read configuration word */
            ...
            if (M.x86.R_BX == pciSlot) {
                M.x86.R_AH = SUCCESSFUL;
		pci_read_config_word(pdev,M.x86.R_DI,&M.x86.R_CX);
        ...
        case 0xB10A:                    /* Read configuration dword */
            ...
            if (M.x86.R_BX == pciSlot) {
                M.x86.R_AH = SUCCESSFUL;
		pci_read_config_dword(pdev,M.x86.R_DI,&M.x86.R_ECX);
        ...
        case 0xB10B:                    /* Write configuration byte */
            ...
            if (M.x86.R_BX == pciSlot) {
                M.x86.R_AH = SUCCESSFUL;
		pci_write_config_byte(pdev,M.x86.R_DI,M.x86.R_CL);
        ...
        case 0xB10C:                    /* Write configuration word */
            ...
            if (M.x86.R_BX == pciSlot) {
                M.x86.R_AH = SUCCESSFUL;
		pci_write_config_word(pdev,M.x86.R_DI,M.x86.R_CX);
                }
            CONDITIONAL_SET_FLAG((M.x86.R_AH != SUCCESSFUL), F_CF);
            break;
        case 0xB10D:                    /* Write configuration dword */
            ...
            if (M.x86.R_BX == pciSlot) {
                M.x86.R_AH = SUCCESSFUL;
		pci_write_config_dword(pdev,M.x86.R_DI,M.x86.R_ECX);
                }
            CONDITIONAL_SET_FLAG((M.x86.R_AH != SUCCESSFUL), F_CF);
            break;
        default:
...
static void  int15(int intno)
{
	...
	case 0x7f07:
		printf("int 15:AX=%x\n",M.x86.R_AX);
		M.x86.R_AX=0x007f;
		pci_read_config_word(pdev,0x0,&M.x86.R_BX);
		pci_read_config_word(pdev,0x2,&M.x86.R_CX);
		break;
	case 0x7f08:
		printf("int 15:AX=%x\n",M.x86.R_AX);
		M.x86.R_AX=0x007f;
...
{
#if 0
    _BE_env.vgaInfo.save_msr = inb(0x3CC);
    _BE_env.vgaInfo.save_vse = inb(0x3C3);
    _BE_env.vgaInfo.save_46e8 =inb(0x46e8);
    _BE_env.vgaInfo.save_pos102 = inb(0x102);
    outb(~0x03 & _BE_env.vgaInfo.save_msr,0x3c2);
    outb(~0x01 & _BE_env.vgaInfo.save_vse,0x3c3);
    outb(~0x08 & _BE_env.vgaInfo.save_46e8,0x46e8);
    outb(~0x01 & _BE_env.vgaInfo.save_pos102,0x102);
#endif
}
...
{
#if 0
    outb(_BE_env.vgaInfo.save_pos102,0x102);
    outb(_BE_env.vgaInfo.save_46e8,0x46e8);
    outb(_BE_env.vgaInfo.save_vse,0x3c3);
    outb(_BE_env.vgaInfo.save_msr,0x3c2);
#endif
}
