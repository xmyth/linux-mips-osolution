/*
 * XG20, XG40, XG42 frame buffer device
	...
	if(isactive) {
		...
		switch(xgi_video_info.video_bpp) 
		{
        	case 8:
                ...
                outXGIIDXREG(XGICR, 0x4D, (cr_data & 0xF8));
#endif
                break;
        	case 16:
...
#if defined(__powerpc__)
                inXGIIDXREG (XGICR, 0x4D, cr_data);
                outXGIIDXREG(XGICR, 0x4D, (cr_data | 0x03));
#endif
			    xgi_video_info.video_cmap_len = 16;
            	break;
        	case 32:
...
#if defined(__powerpc__)
                inXGIIDXREG (XGICR, 0x4D, cr_data);
                outXGIIDXREG(XGICR, 0x4D, (cr_data | 0x05));
#endif
            	break;
		    default:
			    xgi_video_info.video_cmap_len = 16;
...
//	     XGI_Sense30x(); //Yi-Lin TV Sense?
	...
	else if (cr32 & XGI_VB_YPBPR) //yilin: for now output 1080i only
		xgi_video_info.TV_plug = TVPLUG_YPBPR_1080i;
...
static void XGIfb_pre_setmode(void)
{
	...
	switch (xgi_video_info.disp_state & DISPTYPE_DISP2) {
	   ...
	   case DISPTYPE_TV:
		...
		else if (xgi_video_info.TV_plug == TVPLUG_YPBPR_1080i) //yilin for ypbpr output
		{
			cr30 = (XGI_VB_OUTPUT_HIVISION | XGI_SIMULTANEOUS_VIEW_ENABLE);	
			outXGIIDXREG(XGICR, 0x35, 0x60);
		}	
		cr31 |= XGI_DRIVER_MODE;
...
//fix it yilin        outXGIIDXREG(XGICR, IND_XGI_SCRATCH_REG_CR33, (XGIfb_rate_idx & 0x0F));
 outXGIIDXREG(XGICR, IND_XGI_SCRATCH_REG_CR33, 0x00);
...
static char *XGI_find_rom(void)
{
        ...
        char *XGI_sig[6] = {
          "Volari GPU", "XG41","XG42", "XG45", "Volari Z7", "Volari Z9" 
        };
	
	unsigned short XGI_nums[6] = {
	    XG40, XG41, XG42, XG45, XG20, XG21
	};
...
//        printk("XGIfb: Relocate IO address: %lx\n", pci_resource_start(pdev, 2));
  ...
  outXGIIDXREG(XGISR, IND_XGI_PASSWORD, XGI_PASSWORD);
    ...
    inXGIIDXREG(XGICR, 0x4A, CR4A);
    orXGIIDXREG(XGICR, 0x4A, CR4A|0x04);
    inXGIIDXREG(XGICR, 0x48, CR48);
    outXGIIDXREG(XGICR, 0x4A, CR4A);
    if (CR48&0x2)
    {
...
//	printk("\nxgifb:chipid = %x",xgi_video_info.chip);
...
#endif
#endif
//	return(pci_module_init(&xgifb_driver));
