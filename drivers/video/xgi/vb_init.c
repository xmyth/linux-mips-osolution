...
void     DualChipInit(PXGI_HW_DEVICE_INFO, PVB_DEVICE_INFO pVBInfo);
...
BOOLEAN XGIInitNew( PXGI_HW_DEVICE_INFO HwDeviceExtension )
{
   ...
   /* return( FALSE ) ; */
    ...
    /* SetDefPCIRegs */                                   /* alan 12/07/2006 */
    if ( HwDeviceExtension->jChipType == XG27 )
    {
      for( i = 0xD0 ; i <= 0xDB ; i++ )
        XGINew_SetReg1( pVBInfo->P3d4 , i , pVBInfo->pCRD0[i-0xd0] ) ;
      for( i = 0xDE ; i <= 0xDF ; i++ )
        XGINew_SetReg1( pVBInfo->P3d4 , i , pVBInfo->pCRDE[i-0xdE] ) ;
    }
...
/*
    ...
    if ( HwDeviceExtension->jChipType >= XG40 )
    {
       ...
       XGINew_SetDRAMDefaultRegister340( HwDeviceExtension ,  pVBInfo->P3d4,  pVBInfo ) ; /*[BillyLiu]06/11/29 For XG27 A0 */
...
/* Description : */
/* --------------------------------------------------------------------- */
void XGINew_DDR1x_MRS_340( USHORT P3c4 , PVB_DEVICE_INFO pVBInfo)
{
...
/* Description : */
/* --------------------------------------------------------------------- */
void XGINew_DDR2x_MRS_340( USHORT P3c4 , PVB_DEVICE_INFO pVBInfo)
{
    XGINew_SetReg1( P3c4 , 0x18 , 0x00 ) ;
    XGINew_SetReg1( P3c4 , 0x19 , 0x20 ) ;
    XGINew_SetReg1( P3c4 , 0x16 , 0x00 ) ;
...
/* Description : */
/* --------------------------------------------------------------------- */
void XGINew_DDR1x_DefaultRegister( PXGI_HW_DEVICE_INFO HwDeviceExtension ,  USHORT Port , PVB_DEVICE_INFO pVBInfo)
{
    USHORT P3d4 = Port ,
           P3c4 = Port - 0x10 ;
...
/* Description : */
/* --------------------------------------------------------------------- */
void XGINew_DDR2x_DefaultRegister( PXGI_HW_DEVICE_INFO HwDeviceExtension ,  USHORT Port ,PVB_DEVICE_INFO pVBInfo)
{
    USHORT P3d4 = Port ,
           P3c4 = Port - 0x10 ;
...
/* Description : */
/* --------------------------------------------------------------------- */
void XGINew_DDR2_DefaultRegister( PXGI_HW_DEVICE_INFO HwDeviceExtension, USHORT Port , PVB_DEVICE_INFO pVBInfo)
{
    USHORT P3d4 = Port ,
           P3c4 = Port - 0x10 ;
...
void ReadVBIOSTablData( UCHAR ChipType , PVB_DEVICE_INFO pVBInfo)
{
    ...
    if ( ChipType >= XG40 )
    {
        ...
        *pVBInfo->pSR33 = pVideoMemory[ 0x7D ] ;
        ii = 0xF8 ;
        /*
          CR40[ 0][]:CR41, CR40[ 1][]:CR8A, CR40[ 2][]:CR8B

          CR40[ 3][]:CR40[7],CR99[2:0],CR45[3:0]
          CR40[ 4][]:CR59, CR40[ 5][]:CR68, CR40[ 6][]:CR69, CR40[ 7][]:CR6A
          CR40[ 8][]:CR6D, CR40[ 9][]:CR80, CR40[10][]:CR81, CR40[11][]:CR82
          CR40[12][]:CR85, CR40[13][]:CR86, CR40[14][]:CR90, CR40[15][]:CR91
          CR40[16][]:CR92, CR40[17][]:CR93, CR40[18][]:CR94, CR40[19][]:CR95
          CR40[20][]:CR96, CR40[21][]:CRC3, CR40[22][]:CRC4, CR40[23][]:CRC5
        */
                                      /* CR40[0][],CR40[1][],CR40[2][] */
        for( jj = 0 ; jj < 3 ; jj++ ) /* CR41,CR8A,CR8B */
        {
        ...
        ii = 0x118 ;                /* CR40[3][] ~ CR40[23][] */
        for( j = 3 ; j < 24 ; j++ ) /* CR40[7],CR99[2:0],CR45[3:0],CR59,CR68,CR69,CR6A,CR6D,
                                       CR80,CR81,CR82,CR85,CR86,CR90,CR91,CR92,
                                       CR93,CR94,CR95,CR96,CRC3,CRC4,CRC5 */
        {
    ...
    if ( ChipType == XG21 )
    {
        ...
        if ( ( pVideoMemory[ 0x65 ] & 0x02 ) )			/* For XG21 CH7007 */
        {
            ...
            pVBInfo->CHTVVCLKUNTSC = XGI7007_CHTVVCLKUNTSC ;
            pVBInfo->CHTVVCLKONTSC = XGI7007_CHTVVCLKONTSC ;
            pVBInfo->CHTVVCLKUPAL  = XGI7007_CHTVVCLKUPAL ;
            pVBInfo->CHTVVCLKOPAL  = XGI7007_CHTVVCLKOPAL ;
        }
    }
