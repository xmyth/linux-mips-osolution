...
BOOLEAN XGISetModeNew( PXGI_HW_DEVICE_INFO HwDeviceExtension , USHORT ModeNo )
{
    ...
    if ( HwDeviceExtension->jChipType == XG21 )  /* for x86 Linux, XG21 LVDS */
    {
        ...
        if ( (XGINew_GetReg1(pVBInfo->P3d4, 0x38) >> 5 & 0x07) == 0x03 )  /*CH7007 CR38 D[7-5]=011b*/
        {
            pVBInfo->IF_DEF_CH7007 = 1 ;
            HwDeviceExtension->bCH7007 = 1;
            XGINew_SetReg1(pVBInfo->P3c4, 0x30, 0x09);         /* For if (pHWDE->bVGAEnabled)== 0 */
...
*/
    ...
    if  ( !XGI_SearchModeID( ModeNo , &ModeIdIndex, pVBInfo ) || (HwDeviceExtension->SpecifyTiming) )
    {
...
struct factor kind[16]=
{
  ...
  ULONG XGINew_P3cc =  pVBInfo->P3cc;
  ULONG XGINew_P3c2 =  pVBInfo->P3c2;
  UCHAR  b3CC;
      ...
      for(jj=0;jj<=127;jj++) /* (Numerator1) */
      {
          ...
          for(kk=0;kk<=15;kk++)
          {
              ...
              if ( (tempclock >= 150 ) && ( tempclock <= 380) )
              {
                  ...
                  if(vclk > tempclock)
                      temp1= vclk - tempclock;
                  else
                      temp1=tempclock - vclk;
                  if(temp1<min)
                  {
                      clock=tempclock;
                      DeNumerator=ii;
  ...
  switch (b3CC & 0x0c )
  {
      case 0x0 : XGINew_SetRegANDOR(pVBInfo->P3c4, 0x31, 0xCF, 0x10);
                 break;
      case 0x4 : XGINew_SetRegANDOR(pVBInfo->P3c4, 0x31, 0xCF, 0x20);
                 break;
      default  : XGINew_SetRegANDOR(pVBInfo->P3c4, 0x31, 0xCF, 0x00);
                 break;
  }
  XGINew_SetReg1( pVBInfo->P3c4 , 0x2B , (unsigned char) sr2b) ;
  XGINew_SetReg1( pVBInfo->P3c4 , 0x2C , (unsigned char) sr2c) ;
}
               ...
               )
{
  ...
  */
  ...
  XGINew_SetRegANDOR(pVBInfo->P3c4, 0x05, 0x00, 0x86);
  XGINew_SetRegANDOR(pVBInfo->P3d4, 0x11, 0x7f, 0x00);
  XGINew_SetRegANDOR(pVBInfo->P3d4, 0x00, 0x00, (VGAHT & 0xff));    /* HT */
  XGINew_SetRegANDOR(pVBInfo->P3d4, 0x01, 0x00, (VGAHDE & 0xff));   /* HDEE */
  XGINew_SetRegANDOR(pVBInfo->P3d4, 0x02, 0x00, (HDE & 0xff));      /* HBS */
  XGINew_SetRegANDOR(pVBInfo->P3d4, 0x03, 0xe0, (HT & 0x1f));       /* HBE */
  XGINew_SetRegANDOR(pVBInfo->P3d4, 0x04, 0x00, (HRS & 0xff));      /* HRS */
  XGINew_SetRegANDOR(pVBInfo->P3d4, 0x05, 0x60, (((HT & 0x20) << 2) | (HRE & 0x1f)));   /* HRE */
  XGINew_SetRegANDOR(pVBInfo->P3d4, 0x06, 0x00, (VGAVT & 0xff));    /* VT */
  XGINew_SetRegANDOR(pVBInfo->P3d4, 0x07, 0x00, (((VRS & 0x0200) >> 2) | ((VDE & 0x0200) >> 3) | ((VGAVT & 0x0200) >> 4)| ((VGAVDE & 0x0100) >> 5) | ((VRS & 0x0100) >> 6) | ((VDE & 0x0100) >> 7) | ((VGAVT & 0x0100) >> 8)));
  XGINew_SetRegANDOR(pVBInfo->P3d4, 0x09, 0xdf, ((VGAVDE & 0x0200) >> 4));
  XGINew_SetRegANDOR(pVBInfo->P3d4, 0x10, 0x00, (VRS & 0xff));      /* VRS */
  XGINew_SetRegANDOR(pVBInfo->P3d4, 0x11, 0xf0, (VRE & 0x0f));      /* VRE */
  XGINew_SetRegANDOR(pVBInfo->P3d4, 0x12, 0x00, (VDE & 0xff));      /* VDEE */
  XGINew_SetRegANDOR(pVBInfo->P3d4, 0x13, 0x00, (temp1 & 0xff));
  XGINew_SetRegANDOR(pVBInfo->P3d4, 0x15, 0x00, (VGAVDE & 0xff));   /* VBS */
  XGINew_SetRegANDOR(pVBInfo->P3d4, 0x16, 0x00, (VT & 0xff));       /* VBE */

  if ( pXGIHWDE->jChipType == XG21 )
  {
      XGINew_SetRegANDOR(pVBInfo->P3c4, 0x2e, 0x00, ((HRS-1) & 0xff));      /* HRS */
      XGINew_SetRegANDOR(pVBInfo->P3c4, 0x2f, 0x00, (((HRE-1) & 0x3f)<<2) | (((HRS-1) & 0x0300) >> 8));      /* HRS */
      XGINew_SetRegANDOR(pVBInfo->P3c4, 0x33, 0xFE, (((VRS) & 0x01)));      /* VRS */
      XGINew_SetRegANDOR(pVBInfo->P3c4, 0x34, 0x00, (((VRS) & 0x01FE)>>1));      /* VRS */
      XGINew_SetRegANDOR(pVBInfo->P3c4, 0x3F, 0x00, (((VRS) & 0x0600)>>9) | (((VRE) & 0x003F)<<2 ));      /* VRS */
      
  }
  if ( pXGIHWDE->jChipType == XG27 )
  {
      XGINew_SetRegANDOR(pVBInfo->P3c4, 0x2e, 0x00, ((HRS-1) & 0xff));      /* HRS SR2E[7:0] */
      XGINew_SetRegANDOR(pVBInfo->P3c4, 0x2f, 0x00, (((HRE-1) & 0x3f)<<2) | (((HRS-1) & 0x0300) >> 8));      /* HRE SR2F[7:2] HRS SR2F[1:0] */
      XGINew_SetRegANDOR(pVBInfo->P3c4, 0x34, 0x00, ((VRS) & 0x0FF) );      /* VRS SR34[7:0] */
      XGINew_SetRegANDOR(pVBInfo->P3c4, 0x35, 0xF8, (((VRS) & 0x0700)>>8));      /* VRS SR35[2:0] */
      XGINew_SetRegANDOR(pVBInfo->P3c4, 0x3F, 0xFC, (((VRE) & 0x003F)<<2 ));     /* VRE SR3F[7:2] */
      
  }
  if (VerticalActivePixel > 1024) 
  {
    XGINew_SetRegANDOR(pVBInfo->P3c4, 0x0f, 0xf7, 0x08);
  }
  else 
  {
    XGINew_SetRegANDOR(pVBInfo->P3d4, 0x07, 0xef, ((VGAVDE & 0x0100) >> 4));
    XGINew_SetRegANDOR(pVBInfo->P3d4, 0x09, 0xbf, ((VGAVDE & 0x0200) >> 3));
    XGINew_SetRegANDOR(pVBInfo->P3d4, 0x18, 0x00, (VGAVDE & 0x0ff));
  }

  XGINew_SetRegANDOR(pVBInfo->P3d4, 0x11, 0xff, 0x80);
  XGINew_SetRegANDOR(pVBInfo->P3c4, 0x0a, 0xc0, (((VRE & 0x10) << 1) | ((VT & 0x0100) >> 4) | ((VRS & 0x0400) >> 7) | ((VGAVDE & 0x0400) >> 8) | ((VDE & 0x0400) >> 9) | ((VGAVT & 0x0400) >> 10)));
  XGINew_SetRegANDOR(pVBInfo->P3c4, 0x0b, 0x00, (((VGAHT & 0xff00) >> 8) | ((VGAHDE & 0xff00) >> 6) | ((HDE & 0xff00) >> 4) | ((HRS & 0xff00) >> 2)));
  XGINew_SetRegANDOR(pVBInfo->P3c4, 0x0c, 0xf8, (((HRE & 0x20) >> 3) | ((HT & 0xc0) >> 6)));
  XGINew_SetRegANDOR(pVBInfo->P3c4, 0x0e, 0xf0, ((temp1 & 0xff00) >> 8));
  XGINew_SetRegANDOR(pVBInfo->P3c4, 0x10, 0x00, temp);
  ...
  {
     XGINew_SetRegANDOR(pVBInfo->P3d4, 0x14, 0xE0, 0x0f);  
     XGINew_SetRegANDOR(pVBInfo->P3ce, 0x05, 0xBF, 0x0);  
     XGINew_SetRegANDOR(pVBInfo->P3c4, 0x06, 0xE3, 0x10);
     temp = ((pXGIHWDE->Horizontal_ACTIVE / 8 * pXGIHWDE->BPP) / 64) + 1;
     XGINew_SetReg1(pVBInfo->P3c4, 0x10, temp);                        
  }  	
  if (pXGIHWDE->BPP==0x10)
  {
     XGINew_SetRegANDOR(pVBInfo->P3d4, 0x14, 0xE0, 0x0f);  
     XGINew_SetRegANDOR(pVBInfo->P3ce, 0x05, 0xBF, 0x0);  
     XGINew_SetRegANDOR(pVBInfo->P3c4, 0x06, 0xE3, 0x08);
     temp = ((pXGIHWDE->Horizontal_ACTIVE / 8 * pXGIHWDE->BPP) / 64) + 1;
     XGINew_SetReg1(pVBInfo->P3c4, 0x10, temp);    
  }
  if (pXGIHWDE->BPP==0x8)
  {
     XGINew_SetRegANDOR(pVBInfo->P3c4, 0x06, 0xE3, 0x00);
     temp = ((pXGIHWDE->Horizontal_ACTIVE / 8 * pXGIHWDE->BPP) / 64) + 1;
     XGINew_SetReg1(pVBInfo->P3c4, 0x10, temp);    
  }
...
/* Description : */
/* --------------------------------------------------------------------- */
void SetDualChipRegs( PXGI_HW_DEVICE_INFO HwDeviceExtension, PVB_DEVICE_INFO pVBInfo )
{
