...
{ 
   /* emulator x86 cpu ram */
    memset(&M,0,sizeof(M));
    if (memSize < 20480) {
        printk("Emulator requires at least 20Kb of memory!\n");
	return -1;
    }
    if ((M.mem_base = (unsigned long)kmalloc(memSize, GFP_KERNEL)) == 0) {
    ...
    /* sysbios */
    if ((_BE_env.sysmem_base = (unsigned long)kmalloc(0x10000, GFP_KERNEL)) == 0) {
        printk("Out of memory!");
	return -1;
    }
...
void  BE_exit(void)
{
    ...
    if (_BE_env.sysmem_base) kfree((void*)_BE_env.sysmem_base);
}
